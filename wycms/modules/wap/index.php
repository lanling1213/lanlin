...
class index {
	...
	public function search(){
		...
		$end = isset($_POST['end']) && intval($_POST['end']) > 0 ?intval($_POST['end']):5;
		$where = '';
		if ($keyword) {
			$where = " AND n.title LIKE '%".$keyword."%' OR n.description LIKE '%".$keyword."%'";
			
			$sql = "SELECT n.id,n.catid,n.title FROM wy_news n LEFT JOIN wy_news_data nd ON n.id = nd.id WHERE 1 $where order by inputtime desc limit 0,$end" ;
		...
		include template('wap','search');
	}
	public function ajaxSearchList(){
		$keyword = trim($_POST['keyword']);	
		$start = isset($_POST['start']) && intval($_POST['start']) > 0 ?intval($_POST['start']):0;
		$end = isset($_POST['end']) && intval($_POST['end']) > 0 ?intval($_POST['end']):5;
		...
		if ($keyword) {
			...
			$sql = "SELECT n.id,n.catid,n.title FROM wy_news n LEFT JOIN wy_news_data nd ON n.id = nd.id WHERE 1 $where order by inputtime desc limit $start,$end" ;
			$data = $this->db->query($sql);
			$search_detail = array();
			while($res = mysql_fetch_assoc($data)){
				$search_detail[] = $res;
		 ...
		 exit(json_encode($search_detail));
	}
	// 联系我们
	public function lianxi(){
		include template('wap','lianxi');
	}
